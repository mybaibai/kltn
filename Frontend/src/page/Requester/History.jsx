...
import {
  getSosByRequester,
  getVictimProfile as getVictimProfileApi,
...
function getIncidentConfig(typeName, description = "") {
  ...
  if (
    search.includes("sự cố phương tiện") ||
    search.includes("tai nạn") ||
    search.includes("xe") ||
    search.includes("va chạm")
  ) {
    return INCIDENT_CONFIG["sự cố phương tiện"];
  }
  if (
    search.includes("y tế") ||
    search.includes("cấp cứu") ||
    search.includes("sức khỏe") ||
    search.includes("thương") ||
    search.includes("ngã") ||
    search.includes("đau")
  ) {
  ...
  if (
    search.includes("cháy") ||
    search.includes("lửa") ||
    search.includes("nổ") ||
    search.includes("khói")
  ) {
    return INCIDENT_CONFIG["cháy"];
  }
  if (
    search.includes("thiên tai") ||
    search.includes("ngập") ||
    search.includes("sóng") ||
    search.includes("lụt") ||
    search.includes("sạt lở") ||
    search.includes("sạt")
  ) {
  ...
  if (
    search.includes("lạc") ||
    search.includes("mất tích") ||
    search.includes("không tìm") ||
    search.includes("tìm đường")
  ) {
...
function HistoryDetailModal({ item, onClose }) {
  if (!item) return null;
  const { bg, iconClass, Icon } = getIncidentConfig(
    item.incident_type?.name || item.category
  );
      ...
      >
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 flex flex-col gap-4">
          <div className="flex items-center gap-2 flex-wrap">
            <span
              className={`text-xs font-bold px-3 py-1 rounded-full ${statusCfg.className}`}
            >
              {statusCfg.label}
            </span>
            {ai.priority_label && (
          ...
          {ai.situation_summary && (
            <div className="bg-blue-50 rounded-xl px-4 py-3 border border-blue-100">
              <p className="text-[9px] font-bold text-blue-400 tracking-widest mb-1">
          ...
          {ai.victim_advice && (
            <div className="bg-emerald-50 rounded-xl px-4 py-3 border border-emerald-100">
              <p className="text-[9px] font-bold text-emerald-500 tracking-widest mb-1">
                KHUYẾN NGHỊ
              </p>
...
export default function HistoryPage() {
  ...
  const filteredHistory = closedHistory.filter((item) => {
    ...
    if (statusFilter === "completed")
      return key === "completed" || key === "resolved";
  ...
  const cancelledCount = closedHistory.filter(
    (i) => normalizeStatus(i.status) === "cancelled"
  ).length;
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        <aside className="w-52 bg-white border-r border-gray-100 hidden md:flex flex-col justify-between flex-shrink-0">
          <div>
            <div className="px-5 py-5">
              ...
              <div className="text-[10px] text-emerald-500 font-semibold tracking-widest uppercase">
                Emergency Support
              </div>
            </div>
        ...
        <div
          ...
          style={{ background: "rgb(244,251,244)" }}
        >
          <div className="max-w-3xl mx-auto px-8 py-7">
            ...
            <p className="text-sm text-gray-400 mb-4">
              Danh sách các yêu cầu hỗ trợ khẩn cấp đã hoàn thành hoặc đã hủy.
            </p>
            ...
            {loading ? (
              <div className="flex flex-col gap-3">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="bg-white rounded-2xl px-5 py-4 shadow-sm animate-pulse"
                  >
              ...
              <div className="bg-white rounded-2xl px-6 py-12 shadow-sm flex flex-col items-center gap-3 text-center">
                ...
                <p className="text-gray-500 font-medium">
                  {statusFilter === "completed"
                    ...
                    ? "Không có yêu cầu nào đã hủy"
                    : "Chưa có lịch sử cứu trợ"}
                </p>
                <p className="text-gray-400 text-sm">
                  Các yêu cầu đã kết thúc của bạn sẽ xuất hiện tại đây.
                </p>
              </div>
            ) : (
              <div className="flex flex-col gap-3">
                {filteredHistory.map((item) => {
                  const typeName =
                    item.incident_type?.name || item.category || "Sự cố";
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        {/* Info */}
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold text-gray-800 leading-tight mb-1.5">
                            {typeName}
                          </p>
                        ...
                        {/* Status badge */}
                        <span
                          className={`flex-shrink-0 text-[11px] font-bold px-3 py-1.5 rounded-full ${statusCfg.className}`}
                        >
                          {statusCfg.label}
                        </span>
                      </div>
