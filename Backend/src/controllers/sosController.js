...
// Map lưu timer để hủy nếu SOS đã được nhận
...
export const create = async (req, res) => {
  try {
    ...
    // Giữ description sạch + có thể append address
    let desc = typeof description === 'string'
      ? description.trim()
    ...
    io.to('admin-dashboard').emit('sos_created', {
      ...
      victim_id: typeof fullSos?.victim_id === 'object'
        ? fullSos?.victim_id?._id
        : fullSos?.victim_id,
    ...
    try {
      ...
      if (nearRescues.length > 0) {
        ...
        console.log(`📢 SOS ${sos._id} — notified nearest rescue`);
      }
    } catch (e) {
      console.warn('⚠️ Could not find nearest rescue teams:', e.message);
    }

    // ⏱️ Sau 60s broadcast toàn bộ rescue
    const broadcastTimer = setTimeout(() => {
      ...
      console.log(`📢 SOS ${sos._id} — broadcast all (60s)`);
    ...
    // 🤖 AI chạy background (best practice)
...
export const patchVictimLocation = async (req, res) => {
  try {
    ...
    const updated = await sosService.updateSosVictimLocation(
      req.params.id,
      Number(la),
      Number(ln)
    );
